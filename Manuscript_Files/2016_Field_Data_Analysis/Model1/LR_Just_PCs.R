...
#setwd("C:/Users/shensto2/Desktop/Research/Lodging_Data/CFAR_600_Data/Binomial_Data")
home.dir <- getwd()
...
Y <- read.table("cfar_data_PCs_disease.txt", head = TRUE)
...
for(i in 1:nrow(the.SNP.info)){#For loop through all markers
  ...
  pheno.and.this.SNP <- merge(Y, this.SNP, by.x = "Plot", by.y = "GK...1.", all.x = TRUE)
  ...
  #Fit a null model
  nullmod=glm(cbind(Success.lodged, Failure.number_not_lodged)~as.factor(plot_block)+PC1+PC2+PC3,family=binomial(logit),data=pheno.and.this.SNP)	
  
  
  #Fit the logistic regression model
  glm.out = glm(cbind(Success.lodged, Failure.number_not_lodged)~ as.factor(plot_block)+PC1+PC2+PC3+this.SNP, family=binomial(logit), data=pheno.and.this.SNP)
...
}#End for loop through all markers
...
setwd(paste(home.dir, "/Model1_PCs_Disease/", sep = ""))
write.csv(the.results.with.FDR, "Logistic_Regression_Results_disease.csv", row.names = FALSE)
...
GAPIT.Manhattan(GI.MP = the.results.for.manhattan.plot, name.of.trait = "Lodging.Binomial.PC", 
